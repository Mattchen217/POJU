...
`;
  ...
  const analysisBody = sections.find((s) => s.type === "foundation")?.body ?? "";
  assert("parse-delivery finds foundation section", analysisBody.length >= 2, `body=${analysisBody.length}`);
  assert(
