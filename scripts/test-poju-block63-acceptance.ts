...
`;
  ...
  const analysisBody = sections.find((s) => s.type === "talent_map")?.body ?? "";
  assert("parse-delivery finds talent_map section", analysisBody.length >= 2, `body=${analysisBody.length}`);
  assert(
