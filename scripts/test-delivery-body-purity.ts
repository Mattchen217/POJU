/**
 * Delivery body purity gate — zero 命理 in core_conclusion / narrative body.
 *
...
function main(): void {
  ...
  assert("polluted basis dump fails", hit != null);
  assert(
  ...
  assert("finalize gates pollution", finalize.includes("findDeliveryProsePollution"));
  assert("finalize fails any group", finalize.includes("failed.length > 0"));
  assert("narrative gates body pollution", narrative.includes("body_mingli_pollution"));
  assert("narrative fails any task", narrative.includes("failed.length > 0"));
