...
import { DELIVERY_SINGLE_CALL_TIMEOUT_MS, PAGE_SCHEMA_DEEP_WRITE_TIMEOUT_MS } from "../lib/llm/pro/delivery/delivery-tasks";
...
assert.equal(PAGE_SCHEMA_DEEP_WRITE_TIMEOUT_MS, DELIVERY_SINGLE_CALL_TIMEOUT_MS);
assert.equal(PAGE_SCHEMA_DEEP_WRITE_TIMEOUT_MS, 270_000);
assert.equal(SEGMENT_DEEP_WRITE_MIN_INVOKE_MS, 270_000);
...
assert.ok(callSrc.includes("PAGE_SCHEMA_DEEP_ASSIGN_TIMEOUT_MS"));
