...
const file = path.join(__dirname, "../components/poju/ContextSummaryEditor.tsx");
    ...
    <div className="rounded-2xl border border-amber-400/30 bg-amber-950/25 p-4 text-sm shadow-lg">
      ...
      <div className="mt-4 space-y-4">
        {edited.sections.map((section, sIdx) => (
          <motionless key={section.section_id}>
            <p className="text-xs font-medium uppercase tracking-wide text-amber-200/80">{section.title}</p>
                ...
                <li key={item.item_id} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2">
                  ...
                  {editingItem?.section === sIdx && editingItem?.item === iIdx ? (
                    ...
                    <div className="mt-1 flex items-start justify-between gap-2">
                      ...
                      <motionless className="flex shrink-0 gap-1">
                        <button type="button" className="text-xs text-amber-200/90" onClick={() => startEdit(sIdx, iIdx)} aria-label={t("save")}>
                          <span className="material-symbols-outlined text-[16px] leading-none">edit</span>
                        </button>
                        <button type="button" className="text-xs text-red-300/90" onClick={() => deleteItem(sIdx, iIdx)} aria-label={t("confirm_delete_item")}>
...
const fixed = content.replace(/<\/?motionless/g, (m) => (m.startsWith("</") ? "</div" : "<motionless".replace("motionless", "motionless")).replace(/motionless/g, "div");
