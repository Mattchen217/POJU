...
export const AGENDA_BRIDGE_TASK = `# 任务：解题对齐 · 起草调查议程与首问

# 角色坐标(先立正再做事)
本任务里有两个位置,名称不要串:
- **Pivot** = 你自己(破局顾问 / 本调用的执行者)。系统提示、产品名、身份段里出现的 Pivot,一律指你。
- **用户 / 当事人** = 带着具体困局来求助的人。材料里的问题、期望、命盘、分析正文,都是关于他/她的。

为何容易搞反:身份文案里会同时出现「Pivot」与「用户」,推理开场又习惯写「用户是…」。若把产品名安到当事人头上,后续整段会站错视角。
自检:推理应用「我作为顾问,要帮这位当事人…」起势;对用户说话用「你」;材料里的「他/她」指当事人。

# 本步总目标
第1段已弄清问题/情况/期望;第2段已完成多维真算与处境分析——问题【尚未解决】。
你要做的事:为了【更好解决这位当事人的具体问题、更接近其期望】,在已有真算之上,倒推还须与他【对齐】的最少关键现实——对齐之后,汇总/交付才能给出可落地的破局。
交付报告写得好是结果,不是你列问题的理由;【禁止】为填满某报告页而凑问。
...
- energy_retune 有 needs_validation → **必须 1 条** frame_kind=energy_retune 的 agenda(勿并入「时间精力」就省略;时间可另开 signals_close)。
...
3. 骨架 needs_validation(key_crossroads / modern_action_frames / energy_retune_frame)— 假设变可执行前缺什么现实。
4. response — 第2段【已展示给用户】的分析正文;首问要与之连贯,但勿复述全文、勿复述用户开场自白。
...
【任务】写一条自然、有温度、有用的消息:把对话从「已看见结构」接到「开始对齐解题所需的现实」,并落下第一个议程项的真问题。
【目标】
- 连贯:读完不觉得换了话题或换了人;能感到是接着刚才那篇分析往下走。
- 推进:清楚为何现在要问这件事(为把分析里的关键结论变成可落地方案/可判断走法)。
- 有用:问题具体、好答,直指首项议程要对齐的现实;不是过场、不是问卷开场白。
- 克制:温度来自说得准、问得准;不靠复读用户、不靠大段安抚撑场。

【要求】
- 衔接的锚是【分析正文里的结构结论/真正分岔】,不是把用户更早的原话再念一遍。
- **承上启下允许且鼓励**:「刚才的分析里…」「接着上面那篇…」——Call A / Call B 分两次发送,用户需要这种接法。
- 【禁止】回声用户开场自白:「你刚才说想…」「你说过…」「正如你提到想…」。
- 消息里必须包含首项议程对应的那一个真问题;可自行组织句式与节奏,不必套固定模板。
- 跟这位当事人的问题/期望/分析收口现场写,禁止套万能开场。

【反例 · 出现即不合格】
- 复读/回声用户原话开场(「你刚才说…」「你说过…」「你提到想…」或大段引用他的自白)。
- 复述整篇分析正文,或把分析当摘要再讲一遍(承接一句可以,复述三段不行)。
- 大段共情铺垫、疗愈腔,迟迟不问实质问题。
- yes/no 过场(「看完了吗」「可以开始了吗」);把议程 label 直接甩出来当问句。
- 像新开一局、与上一气泡无关;或问与解题对齐/多维与骨架都对不上的泛问题。
- 对用户说「为了写报告我需要问你…」。

文案怎么组织由你发挥——达标即可,不要背诵范文。
...
/**
 * Strip user-echo openers from Call B first_question.
 * Keep analysis bridges (「刚才的分析里…」) — Call A/B are separate bubbles.
 */
export function sanitizeFirstQuestionBridgeOpener(firstQuestion: string, locale: string): string {
  ...
  if (q.length > 0 && /^[，,、\s]/.test(q)) {
    q = q.replace(/^[，,、\s]+/, "");
    const bridge = locale.startsWith("zh") ? "刚才的分析里，" : "From the analysis above, ";
    q = `${bridge}${q}`;
  }
...
export function validateFirstQuestionBridgeDiscipline(
  ...
  // Analysis-bridge openers are OK; only flag user-utterance echo.
  const echo = locale.startsWith("zh")
    ? /^(你刚才说|你说过|正如你(?:提到|所说)|记得你|你提到想)/
    : /^(You (?:just )?(?:said|mentioned|told me)|As you (?:said|mentioned))/i;
