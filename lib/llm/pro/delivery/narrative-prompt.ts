...
- science_action:把 core_conclusion 那套【基于你命理的决策策略】写透——决策边界/发力点/你这类结构的易栽点/主辅切换条件;可落一个「第一步怎么起」的场景示意(用用户说过的现实料锚一下),但【重心是策略,不是替他写执行剧本/话术脚本/专业流程】。禁「谁都适用」的通用步骤(删掉命理依据还成立=重写)。metaphysics_action:推荐方位/高频时段/视觉锚点/协同人群(禁吉凶/属相/风水/玄学字面)。
