...
`
      : primaryKey === "metaphysics_action"
        ? `
# 本页专规 · 东方药方(每项=策略+手段 · 吃满本地算料)
- 整份报告一主一辅;本页只写「东方侧怎样托住主路」——【禁止】再复读科学页的职场折中话术。
- **不要把整页收成「方位+色彩」两三段清单**。须从上游多维 + metaphysics_pack **多维生长**,通常 **3–5 项**,每项 title+strategy+methods。
- 建议覆盖(有料才写,不为凑齐空转;每项仍是策略+手段成套):
  1) 用神喜忌 · 补什么/避什么/怎么借势(对齐主路)
  2) 五行与精力仪表 · 输出/续航/阻力对这件事的打法
  3) 行业/场景属性方向(career themes)
  4) 空间方位适配(为何托住主路 + 怎么用)
  5) 高频时段 / 色彩锚定 / 协同人群特质(可分项或合并,每项仍要策略+手段)
- 手段不限于朝向:时段、视觉锚、协同特质、行业属性都算;策略必须说清「对【他】为何成立」。
- 禁吉凶/属相/风水字面;禁只有手段没有策略。
`
        : "";
...
- **跨页去重铁律**:「养根 / 小森林 / 宜守 / 向内积累」类主隐喻全报告≤1次(只许 foundation)。**禁止各页反复推销同一主路径口号**。
- direct_answer:只给结论头;foundation:论证为什么卡,禁完整路线图;thirty_day:按周双轨,禁谈判逐字稿进周表;risk_guard:结构特有坑;signals_close:正向信号+闭环,禁回来追踪。`;

  const rxDutyRules = `- **每个独立论点单独一项**= title + strategy + methods;严禁把多维塞进同一项。
- **跨页去重铁律**:禁止换皮复读主路径推销句;本页只交本页信息维。
- 详见上方「本页专规」。`;
