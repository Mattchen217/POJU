...
# 铁律(写进 body / scan.value 的内容)
- 正文【纯大白话】【零 ⟦t: 标记】【零干支】——给不懂命理的用户看。
- 【禁词表 · 下列字面禁止出现在 body / scan】(SSOT/合规禁裸词，全表):
  ${bannedList}
- 【表外也不行】凡命理黑话、十神简称、格局/神煞名、干支、支月(寅月等)、用忌短语——即使不在上表,也【禁止】进 body/scan;一律改写成感受/行为/处境大白话。
- 禁软译黑话进 body:锚元/助元/供源/需养/岁环/流展/本元 等——那些只属于「依据与推理」层。
- 【命运红线】禁止字面:命运 / 命定 / 宿命 / 天注定(含否定式诱词)。「判决」可作普通白话;禁「命运判决书」。
- 【禁止自造术语标记】body / scan 里【绝不】写 \`⟦t:…⟧\`(依据层才打标)。
...
- **跨页去重铁律**:「养根 / 小森林 / 宜守 / 向内积累 / 冬天养根」类主隐喻**【全报告最多出现一次】**——通常只允许落在 energy_base 或 macro_cycle 其中一页。其他页必须给**新维度**(十神图谱、神煞阶段、大运窗口、科学动作、玄学五维、红线、信号),禁止换皮复读同一劝导。
- science_action:具体步骤/第一步/可能的坑;metaphysics_action:方位适配/高效时段/色彩锚定/互补伙伴(禁吉凶/属相/风水)。
- energy_base:正面直答 + 解读仪表盘真分 + **三阶段路线图**(1–3月蓄水 / 4–6月松动 / 7–12月吸引,不报日期);【禁止】重画仪表盘数字表(系统已插入结构)。
- macro_cycle:能量周期定性 + 你处于三阶段中的哪一段(前/中/后);禁逐月预测;勿复读 energy_base 的养根金句。
- thirty_day:按周解读双轨动作;【禁止】吐 ASCII 甘特或重画周表(系统已插入结构)。
- signals_close:正向信号 + 一次性闭环;【禁止】邀请回来追踪/订阅。
- talent_map / spirit_gifts / risk_guard:只完成该段职责;禁止用「养根」填满。
- 不做心理诊断标签。
