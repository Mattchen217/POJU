...
const P4_EASTERN_MEAN_SIGNAL =
  /火旺|水旺|金旺|木旺|土旺|用神|忌神|喜神|大运|流年|岁运|运程|未熟|窗口|阶段窗|阶段切换|补给|过耗|借势|以泄代克|角色定位|角色站位|靠近|远离|补泻|泄成|结构节奏|加码|调频|守成|技术输出|站位|以柔克刚|观察者|策略提供者|侧翼|姿态进入|借.{0,6}平台|局势|攻守|仪轨|结界|静润|缓冲冷静|涵养|值使|值符|逆风|未熟不拔|气定|催促场|火阵|静坐|温凉饮/;
...
export function meansFailsDeCalcTest(text: string): boolean {
  ...
  if (
    /火旺|水旺|金旺|木旺|土旺|用神|忌神|喜神|大运|流年|岁运|运程|未熟|窗口|阶段切换|补给|过耗|借势|以泄代克|角色定位|角色站位|靠近|远离|补泻|泄成|结构节奏|加码|调频|守成|技术输出|站位|以柔克刚|观察者|策略提供者|侧翼|姿态进入|借.{0,6}平台|缓冲|结界|仪轨|气定|催促场|火阵|静坐|温凉饮|局势|攻守|未熟不拔/.test(
      t,
    )
  ) {
    return false;
