...
- **禁教练/PM 茎 + 禁通用杠杆类**：律师/文档化/备忘录/书面文档/里程碑/安全垫/收入安全线/保底资金/观察期/缓冲期/试水期限；裸「谈判筹码」无借势/技术输出等机制；以及分散单一依赖、模块化交付换筹码、内心平静后再谈——那是 P3/鸡汤。P4：**优先抄菜单已写死的 means 草稿**（窗口/补给/过耗/借势/守成/调频），只做贴案轻改。
