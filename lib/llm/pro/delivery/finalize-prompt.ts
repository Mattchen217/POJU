...
- science_action:【科学一套】策略(边界/发力/易栽/切换)+手段(资源精力配比/沟通协作原则/节奏杠杆/一层场景示意)。
- metaphysics_action:【东方一套】策略(以用神喜忌:补什么/避什么/怎么借势)+手段(方位/色彩/择时/贵人);用语见切片合规。
...
foundation ← energy_structure + multi_dimension_reckoning(多维) + element_scores + 四柱十神 + 神煞(闭集中性)+十二长生 + 当前能量周期;【论证 P1 的结论:你为什么会卡在这】从【多个命理维度】论证"为什么卡在这"(不只一个点),每维可作一条依据;按【论证需要】放底座料(哪几块支撑「为什么卡」就放哪几块,不为凑齐而凑、不放无关的),内部小标题分块(论证支点),收敛到「所以你卡在这」。仪表盘只用真分;禁逐月预测、禁生肖、禁吉凶;「养根」类主隐喻全报告只在此页用一次。【禁路线图】rhythm 只许一句阶段定位(蓄力/试探/巩固);禁止1–3月路线图、禁止前/中/后10天清单、禁止谈判/授权执行清单(那些归 thirty_day / science_action)。
science_action ← primary_path + backup_path + action_plan + multi_dimension_reckoning(多维) + modern_action_frames + 收集证据;
【生成顺序·药方从命理生长(护城河)】：先从 multi_dimension_reckoning 各维 + 主辅方向，推出「以你的结构，这件事该怎么发力、该守什么边界、你这类结构最容易栽在哪、什么条件下从主路径切到辅路径」——再落【科学手段】(资源/精力怎么配、沟通协作原则、节奏怎么压、一层第一步示意)。**core_conclusion = 科学策略+手段整套**,不是「只有策略」或「先造动作再贴标签」。bazi_basis 填对应各维真词（依据前置）。
【给什么·不给什么(硬判据)】给：【策略】决策边界/发力点/易栽点/主辅切换 + 【手段】可对照的科学杠杆与一层场景示意。不给：替你做的专业执行（合同条款、完整话术脚本、专业流程代做）。手段≠执行剧本。
【出货形状】须同时覆盖策略维与手段维(或等价:角色发力/沟通原则/资源节奏/红线退路);禁止整段「你可以这样开口说…」谈判逐字稿;禁止只有策略没有手段、或只有手段清单没有策略。
【自检】删掉命理依据(bazi_basis)，策略与手段还成立吗？——若照样成立(谁都适用)，就是没扎命理，重写。
辅路径给退路+切换条件(较简)。兜底:无主辅时读 modern_action_frames；多维也缺时，退回主辅+脊柱推——**降级的是料的丰富度，不是「从命理生长」与「策略+手段成套」**。
metaphysics_action ← energy_retune_frame + metaphysics_pack + primary_path(对齐主路);
【东方一套·策略+手段】【策略】以用神喜忌/五行为根:这件事该补什么、避什么、怎么借势(对齐主路径,不是空讲五行课)。【手段】方位/色彩/时段/贵人——每条从用神喜忌推出(缺什么补什么)。禁止只有手段清单没有策略;禁止通用风水吉凶。bazi_basis 填用神/五行真词。禁「先照顾好自己」空话。【自检】删掉命理依据后还成立(谁都适用)=重写。
thirty_day ← rhythm_frame + action_plan + current_da_yun_cycle(当前阶段) + 科学药方动作/东方药方动作按【周】排(4周);【命理扎根】节奏的松紧(哪几周宜守蓄力、哪几周可推进)要能对应他当前的大运/阶段能量,不是把 action_plan 平均切4段;【禁】把 science_action 的谈判逐字稿搬进周表(周表只排节奏与动作类型);勿按天;表由 narrative 的 thirty_day_table 写(非代码拼)
