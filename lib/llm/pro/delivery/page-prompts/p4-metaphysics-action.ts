...
2. **挂 P3 执行面（硬·边界）**: strategy **最多一句**点明「落实主辅/P3 哪类动作时如何调频」；**means 禁止**扩写或复述 P3 谈判/验证期/试水期限/股权条款/文档交付/博客曝光——那些是科学页。调频动作只从【P4 护城河手段候选菜单】来。
3. **每维服务标签（硬）**:维名或 strategy 开篇须标明服务其一——\`主路径推进\` / \`切辅条件\` / \`守成窗口\`。
4. **means 源(硬)**:user 侧【P4 护城河手段候选菜单】已给**完整动作草稿**；每维须**整句抄写+贴案轻改**对应候选 means——禁止另造空槽、禁止无视菜单现编职场课。
...
① 按菜单 eligible 优先探索三类护城河维:
   - **timing** 大运/岁运阶段窗口与策略切换——须写**多久/转折/切换/窗口/守成/加码**（正文可用「较长阶段/这一年/能量交织/未熟」平替，禁只写情绪安定）
   - **polarity** 用神忌神驱动的靠近/远离与补泄取舍
   - **archetype** 十神/格局驱动的身份与角色定位——**禁止**与 timing 维文案逐字雷同
② 对照问题+期望:哪些候选能改他落实主辅时的**身心/节奏/站位**处境?
...
- 挂 P3 时只写「做该动作时如何调频」，不是重写 P3 的安全线/备忘录/律师/文档交付/验证期条款。
...
- ❌ 职场教练/PM 茎:律师/文档化/备忘录/书面文档/里程碑/安全垫/收入安全线/保底资金/观察期/缓冲期/试水期/试水期限/验证期/财务 KPI/个人博客曝光课；裸「谈判筹码」无借势/角色机制。
