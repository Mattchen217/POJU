...
5. **三柱（正文零裸专名，机制必须在；内部 type 仍 timing|polarity|archetype）**：
   ...
   - **站位·仪轨（archetype + 白名单动作）**：借势不硬刚；静坐/温凉饮/清静场/背靠实墙等仪轨。**禁**「看清条款/谈股权/写合同」——改写「看清底线、守结界」。
6. **厚度**：strategy ≥${P4_MIN_STRATEGY_SENTENCES} 句且 ≥${P4_MIN_STRATEGY_CHARS} 字；means≥${P4_MIN_MEANS_PER_DIM}；**dimensions 条数恰好=派工锁定表**（禁自增维）。
7. **文风**：东方处世谋略感；**禁**投入带宽、补给态、过度激活、破窗加码、独处降噪（菜单已给替代词则抄菜单）；**禁**「奇门锁盘显示」专名报幕；**禁**合同/条款/股权/律师/Excel/OKR（P3 工具词族，命中即废）；**禁**口播话术剧本（「我考虑一下…」）与「用技术文档/架构说明回应」「找律师守结界」——手段只抄菜单局势/意象/仪轨。
