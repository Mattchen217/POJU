...
# 目标
strategy = 该条批断的机制白话，零命理词；须覆盖下方「本卡译出义务」里的结构类。
means = 同一机制长出的可动手杠杆（删掉批断后垮掉）。同页各卡手段必须可分辨，禁止塌成同一套动作话术。

# 必填槽
- page="science_action": page_title, page_subtitle, **primary_toolkit + backup_toolkit**。
- **angles 钉死 3+3**，顺序与锁定单元 path 对齐。
- 每个 angle: name + **strategy(2–3短段,空行分隔)** + means(1–3)。
- strategy 只展开该条批断里的结构链；禁止访谈原句、处境决策句。
- means 须能指回该条译出义务（合局/互耗/运岁窗/通关/疏导/补源/润化等，白话）；禁止批断未写的生活域故事。
- chart_anchors 留空。禁止 alert。
- page_title/subtitle 概括各卡结构主题差，禁止复述用户问题决策句。

# 禁区（硬 · 换人换盘换题仍成立）
- **零命理词**：禁生水/生火/制火/泄土/泄秀/调候/喜用/用忌/用神/忌神/贵人支持（含引号偷渡）。
- **禁跨卡塌缩**：批断结构类不同（合局 vs 相害 vs 运岁窗 vs 助燃改润化）时，正文不得写成同一套「输出+降温」复读。
- **禁平行生活故事**：批断没有的资产变现/投融资/甘特项目管理/疗愈课等域，禁止写进 strategy/means。
- **禁空壳隐喻墙**：禁止整段用系统过热/冷却液/冷却机制/炉膛/排气阀代替本卡结构链；比喻≤1句。
- **禁通用疗愈清单**：静坐/深呼吸/冥想/考证/固定睡眠。
