...
# 目标
strategy = 该条批断的机制白话（外部加压/泄压通关/根基互耗/资源过热/调节力被泄/短期合局窗口/长期助燃改润化），零命理词。
means = 同一机制长出的可动手杠杆（仍须删掉批断后垮掉）。六卡手段必须彼此不同，各点名本卡批断特有动作。
...
- means 须能指回该条批断的通关/泄压/护根/降温/合局窗口/润化动作（白话）；禁止「今晚起草提案 / 股权表 / 律师模板 / 模拟谈判」；禁止静坐/深呼吸/冥想/考证/固定睡眠一类通用疗愈清单。
- chart_anchors 留空。禁止 alert。
- page_title/subtitle 概括六条结构主题，禁止复述用户问题里的决策句。

# 禁区（硬）
- **零命理词**：禁生水/生火/制火/泄土/泄秀/调候/喜用/用忌/用神/忌神/贵人支持；也不许加引号偷渡。
- **禁空壳隐喻墙**：禁止整段用「系统过热/冷却液/冷却系统/排气阀/机器高温/往火里添柴」代替本卡结构链。比喻最多一句，且后面必须回到该条批断的机制。
- **禁六卡复读「技术输出」**：食神/输出疏导只在批断真有泄秀时点一次；其他卡写本卡自己的加压/互耗/窗口/助燃。
