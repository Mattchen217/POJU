...
5. **每段/每字段 ≤120 词（中文 ≤180 字）**；**同一个术语，body 里每一次出现都要套 ⟦t:…⟧**（因为每一处的语境白话不同、各自可点）；唯一例外：同一段落里同一个词【紧挨着重复】出现（同一语境、白话会一样），只标第一次。**绝不在别处套过一次后，后文把同一个词裸写。** 全文**一个主比喻**，tooltip 小比喻与之呼应（瘦身四条）
