...
{
  "question_response": "80-130 字【答案先行·唯一复述处】。第一句复述用户问题，随后 2-4 句给出镜子照出的方向：此刻的势在助还是在拦、关键着力点是什么。精炼、不展开依据、不堆术语。",
  "wind_category_blurb": "40-70 字。这个风类的整体氛围（不提问题；六条红线）",
  "classical_voice": "120-200 字。签的意象与《易经》时位（不提问题；archetypal metaphor 开篇一次；用 Glyph 指代）",
  "命理双视角": {
    "命理看此事": "280-500 字【依据·命盘，不复述问题】。日主/大运/用神/五行（允许术语，就近用大白话解释）看此人对此事的天然倾向、亮点与盲点。直接分析，不复述问题。",
    "签文看此事": "280-500 字【依据·签象，不复述问题】。Glyph 文原型对此事照出的意象（可引用 1-2 句签诗原文，抽象化；用 Glyph 指代，禁 sign/lot）",
    "两者印证或冲突": "140-260 字。两面镜子如何印证或形成张力（不复述问题；六条红线）"
  },
  "synthesis": "280-450 字【深度整合·严禁复述问题、严禁重复 question_response】。把命盘×签象×问题拧成一个更深的洞见：点名关键变量、说清条件、给出着力点；可含能量层面的时机窗口感（不报日期）。默认用户已知问题，直接深入。",
  "hidden_tension": "100-160 字。盲点或暗流（当下视角）",
  "your_moment": "120-180 字。当前年度能量节律 + 能量层面时机窗口感（顺势/沉淀，禁报日期、禁预测事件）",
  "exploration": {
    "text": "100-160 字微练习（见 exploration 专节）：何时+场景+动作链+尽量有产出物；形态多样，禁止默认「安静坐下闭眼+纸上书写」",
    "timeframe": "today | tonight | within_24h | this_week",
    "duration_estimate": "X minutes",
    "is_solo": true
  },
  "reflection_question": "60-90 字反思问句（邀请非命令）",
  "invalid_input": false,
  "_meta": {
    "glyph_id": ${glyph.id},
