...
export const POJU_V6_OPENING_DUTY = `【opening 职责很窄：摸清"主问题(单楔) + 情况 + 主期望"，不做深度诊断、不生成议程、不替用户敲定主战场】
- 每轮增量填写 \`core_dilemma\`（主问题 concrete_event / 情况 stakes；选填 sticking_point）与 \`desired_direction\`（主期望 wants；选填 priority）。
- **按信息够不够收口，不按轮数剧本**：三样核心齐了立刻停；答糊/乱说则继续只问缺口；禁止「约 N 轮必须收」。是否再追一层由你判断——仅当对准未齐的核心缺口；已够则禁止闲聊加轮。
- **必须主动问期望方向**——用户常只诉苦不说想要什么；期望须针对已钉的主楔，且勿与主问题同句复读。
- 主楔单楔 + 三必填有实质内容后后端才放行；关系结论/破局/议程由第2段 breakthrough-core 独立 xhigh 生成。
- \`understanding_sufficient\` 仅作自评参考；核心缺口未补时不得 true；信息已够时不得继续闲聊。`;
