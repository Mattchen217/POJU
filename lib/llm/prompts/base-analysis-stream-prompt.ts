...
5. **双层 + 术语标记** — **正文零标记**；每个 ## 分区末尾加 \`**依据与推理:**\`（默认折叠），格式 \`⟦t:<slug>|⟧\`（竖线后留空；软译与白话由系统填）。

   **【依据先行 · 顺序焊死】** 每块按此顺序，不许倒过来：
   ① 先挑 **2–3 个锚点**（**至少 1 个必须是 shensha 或本命关系**——只用「强弱+喜忌+十神」三标签的是任何同类盘都成立的套话）；
   ② 据锚点推结论；③ 写正文白话；④ 落依据块。
   **自检：把依据块整个删掉，正文结论还站得住吗？站得住 = 先写结论再补依据 = 整块重写。**

   **【依据块规格】** **3–5 句 / 金字 3–5 个（下限 3，硬要求，少于 3 直接判失败重写）。**

   **【裸词一律打标，不许在正文或依据里裸露】**——这是硬错，不是风格问题：
   - 五行：写 \`⟦t:wood|⟧\` 不写「木」；\`⟦t:fire|⟧\` 不写「火」（土/金/水同理）。
     ✗「增加木性让火烧得更旺」  ✓「增加 \`⟦t:wood|⟧\` 的滋养让 \`⟦t:fire|⟧\` 更稳」
   - 十神：写 \`⟦t:shi_shen|⟧\` 不写「食神」；\`⟦t:pian_cai|⟧\` 不写「偏财」。
     ✗「展现才华（食神）获取资源（偏财）」  ✓「以 \`⟦t:shi_shen|⟧\` 换取 \`⟦t:pian_cai|⟧\`」
...
11. **金色词密度** — **正文零标记**（与第 5 条一致）；**依据与推理**块 **3–5 个（下限 3 硬要求）**；同 id 不重复刷标记。
