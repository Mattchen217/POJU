...
- **bazi_basis（命理依据清单）**：支撑这个结论的命理依据，列成字符串数组。
  ...
  **写进 JSON 时再把每条翻成"术语保留 + 白话连接"的成品**——行话草稿不输出。
  ...
  ②【用初中生能懂的大白话连接】：术语之间的连接词,必须是初中生一读就懂的大白话。

  判断标准(每写一句都用这个标准检查):
  想象你把这句话念给一个【完全不懂命理的初中生】听。
  术语(日主、正印、比肩这些)他不认识没关系——那是专业词,会标注解释。
  但【连接术语的每一个词】,他都必须听得懂。
  ...
  · 合冲刑害类:半合、三合、六合、合化、合身、争合、妒合、拱(拱木/拱局)、
              相冲、相刑、相害、相破、自刑、冲开
  · 制化泄耗类:制杀、化杀、泄木气/泄火气(泄X气)、耗、夺、通关、调候、疏、纳、通、化、制、克、泄
  · 旺衰状态类:得令、失令、当令、司令、通根、无根、有根、得地、失地、透干、透出、藏干、
              入墓、坐(坐辰)、临(临官)、长生、帝旺、衰、病、死、墓、绝、旺、相、休、囚
  · 格局强弱类:身强、身弱、身旺、偏强、偏弱、从格、专旺、党众
  · 喜忌取用类:喜用、为用、为忌、取用、用之、忌之、畏、怕
  ——【不止上面这些】。判断一个词要不要翻,不是看它在不在上面的清单里,
    而是用最上面那个标准:"初中生听得懂吗?"听不懂就翻。
    【绝不要】以为某个行话"已经够通俗了"就不翻——只要不是日常说话会用的词,就要换。

  下面给你【一个例子】,让你明白要翻到什么程度(只示范"程度",不要照抄内容):
    行话(你脑子里算的):  "日主乙木得亥水正印生扶,又得比肩劫财帮身,亥未半合木局,身强"
    白话(你要输出的):    "日主乙木生在辰月,年支的亥水(正印)在滋养它,
                         年干和时干的木(比肩、劫财)也在帮衬它,
                         地支的亥和未又聚成一股木的力量,所以它的能量很强"
    ——看到了吗?术语(日主/正印/比肩/劫财)全保留,
      但"生扶→滋养""帮身→帮衬""半合木局→聚成一股木的力量""身强→能量很强"
      这些行话,全换成了初中生能懂的话。你要的就是这个程度。

  【禁止】把行话原样列进去,如"大运逢木火喜用""日主偏弱需印扶""印星化杀需调整"
    "得正印生扶""劫财帮身""半合木局""泄木气""制木"这种紧凑行话——
    这是你脑子里的推理,不是给下游的成品。
...
4. **bazi_basis 写的是白话成品，不是推理草稿**：推理可用行话，写进数组必须是
   「术语保留 + 白话连接」；凡初中生听不懂的行话连接词(生扶/帮身/半合木局/泄X气…)
   一律禁止原样入数组——用上面②的标准与程度例子自检。
