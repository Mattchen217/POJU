...
| 层 | 来源 |
|---|---|
| **固定标签** | 前端 `DELIVERY_PAGE_TAGS`（核心直答 / 归因剖析 / 破局策略 / 东方谋略 / 风险预警 / 行动建议） |
...
- **P4 东方谋略（暗锦囊）**：锚定用户**问题+期望**；双核真算（八字+奇门锁盘）；正文三柱=局势交锋·意象调频·行为仪轨；用户可见层零裸专名、软译 SSOT；**硬禁**复读 P3 合同/股权/律师/文档工具；折叠依据层闭集真算（含奇门结构）；规格锁见 `P4-东方谋略-规格锁.md`
- **切页轴**：按域（破局策略 vs 东方谋略），不按「策略页/行动页」；每页内策略+行动成套

仪表盘 `score` **只**来自 `metaphysics_pack`；禁止模型编造。

### P4 用户可见合规包装

Stripe/PayPal/微信支付宝审核主要扫默认 DOM 与截图。本页默认可见层（维标签、gloss、展开按钮、维名、strategy/means）**禁止**玄学/命理/八字/五行/用神/风水/运势/奇门遁甲裸词报幕。

| 层 | 口径 |
|---|---|
| 固定标签 | 「东方谋略」（`DELIVERY_PAGE_TAGS.metaphysics_action`） |
| UI chrome | 「策略维」+ gloss「局势/气场/仪轨」；**仅 P4** 依据展开「你为什么能这么做」；其余页仍用「底层依据」 |
| `dimensions[].name` | 东方谋略语感命名（局势/意象/仪轨），禁 P3 工具词 |
| 折叠证据 | 闭集 `⟦t:⟧` 真算保留（八字+奇门）——删双核依据应垮 |

内部 segment key 仍为 `metaphysics_action`；pack + 奇门 Fact-pack（Step1）映射见规格锁。
...
| P3 | multi_dim + 主辅 + action_plan + **pack 结构极性** | pack 禁写成 P4 谋略清单 |
| P4 | multi_dim + retune + 整包 pack + **奇门 Fact-pack** + 问题/期望 | 双核；用户可见软译；依据闭集 |
...
- **P5 熔断（执行刹车）**：盯住 P3/P4 Action Brief 手段；结构特有红灯/坑/切辅/防护；每条先规划四点再由模型写成 **narrative** 温暖段落（UI 只展示 narrative，禁止代码拼接四点）；**每条 RiskItem 独立 chart_anchors + 独立依据槽**；禁另立与药方脱节的议程/时限 KPI；生长顺序=先熔断锚→再 Brief 手段→再 narrative
