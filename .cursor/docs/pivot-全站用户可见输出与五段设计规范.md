...
| 项 | 要求 |
|---|---|
| **目标** | 动态对话收齐：主问题(单楔) / 情况 / 主期望；不做深度命理诊断、不产议程、**不替用户敲定**主战场 |
| **节奏** | **按信息够不够收口，不按固定轮数**：三样核心齐了立刻停；答糊/乱说则继续只问缺口；禁止「约 N 轮必须收」；是否深挖由模型判断——仅对准未齐核心缺口，已够则禁止闲聊加轮 |
| **用户可见** | **仅** `response` 进气泡；`options` 在输入区芯片。`stakes`/`concrete_event`/`wants` **不上屏**（确认卡再展示结构化摘要） |
| **多议题首包** | `response` 须：总命名 + 分面点名（用他的具体词）+ 收窄一问；禁止只有口号两句；厚度写在 response，不靠渲 stakes |
...
| **主问题** | `concrete_event` = 用户主要想**先**解决的那一件；多议题其余面进 `stakes` 作背景；点选 options = 用户表态 |
| **期望够用线** | `wants` = 针对主楔的结果**方向/状态**即够；**不是**方案形态；**不得**与主问题同句复读后收口 |
| **手段边界** | 手段是命理**产出**，第1段**不向用户收集**；「了解想到哪」本分，「追怎么做到」越界 |
| **禁止** | 替用户敲定主战场后收口；多议题点选战场同轮把选项原文填进 wants 就收口；多议题并列当主问题收口；为方案具体化继续追；清空已填 wants；三样已齐仍闲聊加轮；复读用户原话；灌映射全表；展开命盘分析 |

关键文件：`lib/llm/phases/opening-phase-v6.ts`（规则）+ `lib/llm/prompts/poju-base-v6.ts`（`POJU_V6_OPENING_DUTY`）
