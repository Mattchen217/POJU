...
> **现行尺（2026-09-20 起）**：旧的「一卡一词、mark 打标润色、金字 ≤3」**不再作为放行条件**。下文第 2 节起的历史签字只作档案。放行以本节为准，并与 `.cursor/docs/交付报告-先批断后正文-开发文档.md`、规则 `13-delivery-judgment-first.mdc` 一致。
...
| write / write.merge | 每条是多词命理批断，合并后没被砍薄 | 一个金字 + 大白话；或飘到别的盘 |
