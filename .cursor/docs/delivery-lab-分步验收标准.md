...
> **现行尺（2026-09-20 起）**：旧的「一卡一词、mark 打标润色、金字 ≤3」**不再作为放行条件**。下文第 2 节起的历史签字只作档案。放行以本节为准，并与 `.cursor/docs/交付报告-先批断后正文-开发文档.md`、规则 `13` / `15` 一致。闸门定尺后主调模型输出，见 `15-delivery-gate-freeze-tune-model.mdc`。
...
2. **再把这份批断译成正文**（该页所有用户可见正文：白话，零命理词）。表象 / 本质只是 P2 的举例，不是只改这两个字段。P3 手段、P4 维度叙事、P5 熔断叙事、P6 收束同样是正文。删掉批断后，正文不得独自成立。
...
| write / write.merge | 无标记多词批断；只证本卡主张；生克落闭集表；地支十神用本气；无白话尾巴 | 一个金字 + 大白话；主张外合冲；生克写反；处境/职业白话 |
| fill | 本页全部用户可见正文都是该条批断的翻译，零命理词（不限 P2 的表象/本质） | 正文与批断无关，或正文里出现命理词 |
| mark | 依据仍是原样批断 | 被打成软标或改写成白话 |
| P1 fill | 直答问题；没有先写的批断就不挂依据 | 空挂一段依据 |
| assemble | 六页通读；有依据的卡片符合上面三层 | 依据只是多一段正文 |

Lab 页每一步标题下会显示同一句「本步合格」。用旧盘逐步跑，看输出，不合格就改提示词/喂词后再跑这一步，不要跳到下一步碰运气。
