...
**命中需要同时满足三件事：**
1. **同一供应商节点**（靠 `session_id` 做 sticky 路由）。
...
| 环节 | 位置 | 作用 |
|---|---|---|
| sticky 路由 | `lib/llm/openrouter-shared.ts` → `openRouterRequestExtras(session_id)` | 把 `session_id` 写进请求体，相同 id 路由到同一节点 |
| **禁止项** | 同上 → `openRouterProviderExtras()` | **绝不设 `provider.order`**——一旦设了就关闭 sticky 路由 |
| session key | `lib/llm/cache-session-id.ts` | 各产品的 sticky key 生成器（见下表） |
| 命中观测 | `openRouterChatCompletion()` → `cached_tokens` + `console.log("[openrouter] cache hit …")` | 命中会打日志，含 `cached_tokens / prompt_tokens / session` |
...
### INV-3 `session_id` 稳定且一致
- 用 `cache-session-id.ts` 的生成器，**勿在路径间用不同 key**。
  - ⚠ 已知不一致：POJU **phase 路径**当前传原始 `session.session_id`，**legacy 路径**用 `pojuCacheSessionId(...)`。应统一为 `pojuCacheSessionId(...)`。
- **绝不设 `provider.order`**（关闭 sticky 路由）。`OPENROUTER_PROVIDER_IGNORE` 只用 `ignore + allow_fallbacks`，不碰 `order`。
...
2. **确认供应商**：sticky 路由把同 session 固定到同一上游。若供应商被切换（如禁用某 provider），缓存随之失效一次。
3. **确认 `provider.order` 没被设**（INV-3）。
...
- [ ] `session_id` 生成器是否仍一致、没设 `provider.order`？（INV-3）
