...
interface SyncroAlgorithmOutput {
  ...
  directions: Array<{
    ...
    overall_score: -2 | -1 | 0 | 1 | 2;  // -2=最不利, +2=最有利
  }>;
}
```
...
For each of 12 directions, generate:
...
Return JSON with exactly 12 entries:
...
Total output: ~600-900 words across 12 directions.
...
The user only sees the compass direction (N, NNE, etc.) 
and your guidance text.
...
计算:
  ...
  Step 4: 12 地支方位各自的"元素"已知(固定)
    - 子: 水, 丑: 土, 寅: 木...(完整列表已在 Prompt v2 文档)
  
  Step 5: 计算每个方位 vs 用户 + vs 当前的双重关系
    最终分数 = 0.6 × 用户关系 + 0.4 × 当前关系
